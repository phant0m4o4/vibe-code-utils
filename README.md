...
- [`prompts/`](prompts/) — 提示词模板
  - [`new-project.md`](prompts/new-project.md) — 从零启动(或接入已有)项目的完整托管模板,按 **脚本 / 小工具 / 正式产品** 三档分级。覆盖领域文档生成、治理文档(AGENTS.md / PLAN.md)、ADR、CI/CD、迭代实施等全流程,内置走样防护(SoT、walking skeleton、并行准入、反幻觉补全、求助退路、回滚约定),并吸收 [Karpathy 编码原则](https://github.com/forrestchang/andrej-karpathy-skills) + 边界与耦合约束作为代码级行为约束。
...
- **项目目录**:空目录(新建)或已有代码目录(自动进入 retrofit 模式)
- 在对应目录打开 AI 助手
...
| 阶段 | 你做什么 | agent 做什么 |
|---|---|---|
| **0 前置 + 提问** | 回答 4 问:项目一句话定位 / 规模(脚本 / 小工具 / 正式产品)/ 有无 UI / 是否对外 API | 检查目录、识别新建 vs retrofit、列出对应规模档的文档清单请你确认 |
...
| **3 迭代实施** | 给出业务方向、回答求助、决定何时 `git push` | 按 walking skeleton 推进:先 CI/CD 骨架 → 端到端最简路径 → 分模块加深;每步自动 commit、归档 PLAN.md,**push 永远手动** |
...
- ⚙️ **可对接已有项目**:已有代码目录自动进入 retrofit 模式,先盘点 `inventory.md` 再补齐缺失文档,不假装从零
...
| **C 正式产品** | 多人 + 上生产 + 长期演进 | 10+ 份 + 强制字段 | 全部(A1-A4 / B1-B5 / C1-C5 / D1-D5) |
