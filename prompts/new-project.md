...
- `tech-stack.md`:语言、Web/核心框架、数据库(若有持久化)、包管理
- `requirements.md`:核心数据模型的**字段清单**、核心 endpoint 的**完整列表**
- `architecture.md`:模块拆分、数据流方向、关键依赖
- 档 C 额外:SLO/SLA、错误率/延迟上限、密钥管理方案

非关键字段(性能指标的具体数值、边界场景的细节、deployment 平台等)允许 TBD 进阶段 2,在阶段 3 对应 step 补齐。

**判定**:扫描所有 SoT 文档,若任一关键字段仍为 TBD 或 [假设],主代理**显式停下**,问用户该字段;**禁止**自填后说"待用户审查"——这是让幻觉溜过验收 gate 的常见路径。
