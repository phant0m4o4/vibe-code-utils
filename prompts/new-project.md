...
12. **反幻觉补全**——**严格区分"用户明示" vs "我推断"**。明示事实直接填入;**未明示字段**留 `TBD` 或写 `> [假设] <推断>` 显式标注。**禁用**"项目类型常见做法"自动补全(默认覆盖率、主流框架、JWT 鉴权、常见目录、典型 schema 等);即使值看起来很合理也不行,合理 ≠ 用户要的。子代理 brief 必含**用户已明示事实清单**;每文档生成后**主代理立即提取所有 [假设] 和 TBD,列给用户逐条审查**(确认/修改/补全),通过后才进下一波——这是验收 gate 的具象化形式。
